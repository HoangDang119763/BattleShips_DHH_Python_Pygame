...
class Player:
    ...
    def makeAttack(self, grid, logicgrid, game):
        ...
        if grid[0][0][0] < posX < grid[0][-1][0] + game.size.cellSizeY and grid[0][0][1] < posY < grid[-1][0][1] + game.size.cellSizeY:
            for i, rowX in enumerate(grid):
                for j, colX in enumerate(rowX):
                    if colX[0] < posX < colX[0] + game.size.cellSizeY and colX[1] < posY < colX[1] + game.size.cellSizeY:
                        if logicgrid[i][j] != ' ':
                            if logicgrid[i][j] == 'O':
